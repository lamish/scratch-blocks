...
goog.provide('Blockly.Dialogs');
goog.require('Blockly.DialogDiv');
...
Blockly.Dialogs.generateModuleDialog = function (module, ballList, multiMode, focusSeq, onChange) {
    console.log(`module`, module)
    
  Blockly.Dialogs.module = module;
  switch (module) {
...
Blockly.Dialogs.flashCurBallLight = function(focusSeq) {
  console.log(`flashCurBallLight`)
  let event = new CustomEvent('mabot', {
    detail: {
        type: 'mabot_flash_light',
        params: {
          seq: focusSeq
        }
    }
...
Blockly.Dialogs.closeCurBallLight = function(focusSeq) {
  console.log(`closeCurBallLight`)
  let event = new CustomEvent('mabot', {
    detail: {
        type: 'mabot_close_light',
        params: {
          seq: focusSeq
        }
    }
...
Blockly.Dialogs.feedBack = function(focusSeq, state, ballCnt) {
   ...
   switch (module) {
    case Blockly.FieldModuleDialog.MODULE_MOTOR:
        if(state) {
          Blockly.Dialogs.flashCurBallLight(focusSeq);
        } else {
          Blockly.Dialogs.closeCurBallLight(focusSeq);
        }
        break;
    case Blockly.FieldModuleDialog.MODULE_WAIST_JOINT:
        if(state) {
            console.log(`motion_horizontalJoint_feedback`)
                event = new CustomEvent('mabot', {
                detail: {
                    type: 'motion_horizontalJoint_feedback',
                    params: {
                      seq: focusSeq
                    }
                }
    ...
    case Blockly.FieldModuleDialog.MODULE_ARM_JOINT:
        if(state) {
            console.log(`motion_swingJoint_feedback`)
            event = new CustomEvent('mabot', {
            detail: {
                type: 'motion_swingJoint_feedback',
                params: {
                  seq: focusSeq
                }
            }
...
Blockly.Dialogs.addDashedCircleSvg_ = function (parent, isYellow) {
  ...
  svg.style.top = '0px';
  svg.style.left = '0px';
  svg.style.transform = `rotate(52deg)`;
...
Blockly.Dialogs.generateBottomPane = function (ballList, multiMode, focusSeq, onChange, ballCnt = 15, mBallList) {
  ...
  gridPane.style.backgroundColor = '#32374b';
  ...
  for (var i = 0; i < ballCnt; i++) {
    ...
    */
    ...
    dot.style.height = '3rem';
    dot.style.borderRadius = '1.5rem';
    dot.style.backgroundColor = ballList.indexOf(i + 1 + '') > -1 ? '#01cf95' : '#576390';
    dot.textContent = i + 1;
    dot.style.color = 'white';
    dot.style.textAlign = 'center';
    dot.style.lineHeight = '3rem';
  ...
  okButton.setAttribute('src', Blockly.utils.getRuntimeImagePath('dialogs/determine.png'));
  ...
  eventWrappers.push(
    Blockly.bindEvent_(gridPane, 'mouseup', null, function (e) {
      ...
      if (target.parentNode.parentNode && target.parentNode.parentNode.parentNode === gridPane) {
              ...
              if (focusSeq == itemIndex + 1) {
                ...
                if (ballList.length > 1 || (Blockly.Dialogs.canBallCancel && Blockly.Dialogs.canBallCancel(!!mBallList)) ) {
                  ...
                  if (nextAutoIndex > 0) {
                    ...
                    nextDot.style.backgroundColor = '#01cf95';
...
Blockly.Dialogs.generateModuleMotorDialog = function (ballList, multiMode, focusSeq, onChange) {
  // ballList = JSON.parse(JSON.stringify(ballList)); // simple deep clone
  // ballList = ballList.sort(function(a, b) { return a - b; });// ASC
  // focusSeq = focusSeq || '1';
  // var focusIndex = ballList.indexOf(focusSeq);
  // if (focusIndex < 0) throw 'Invalid focus index: ' + focusSeq;
  // var eventWrappers = [];
  ...
  //         }
  //       }
  //       ballList = ballList.sort(function(a, b) { return a - b; });
  //     }
  //   })
  // );
...
// 阻塞非阻塞
Blockly.Dialogs.generateModuleBlockDialog = function (src, onChange, defaultCurrent) {
  var eventWrappers = [];
  // 当前状态
  // var current = (src.split("/images/")[1] == 'blk_ic_blocked.png') ? 0 : 1;
  console.log(defaultCurrent, "__defaultCurrentdefaultCurrentdefaultCurrent");
